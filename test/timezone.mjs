...
describe('TimeZone', () => {
  ...
  describe('sub-minute offset', () => {
    const zone = new Temporal.TimeZone('+00:19:32');
    const inst = Temporal.Instant.from('2000-01-01T12:00Z');
    const dtm = Temporal.PlainDateTime.from('2000-01-01T12:00');
    it(`${zone} has ID ${zone}`, () => equal(zone.id, `${zone}`));
    ...
    it(`(${zone}).getPlainDateTimeFor(${inst})`, () =>
      equal(`${zone.getPlainDateTimeFor(inst)}`, '2000-01-01T12:19:32'));
    it(`(${zone}).getInstantFor(${dtm})`, () => equal(`${zone.getInstantFor(dtm)}`, '2000-01-01T11:40:28Z'));
    it(`(${zone}).getNextTransition(${inst})`, () => equal(zone.getNextTransition(inst), null));
    it(`(${zone}).getPreviousTransition(${inst})`, () => equal(zone.getPreviousTransition(inst), null));
  });
