...
function ToSyntacticallyValidMonthCode(valueParam: unknown) {
  const value = RequireString(ToPrimitive(valueParam, StringCtor));
  if (
    value.length < 3 ||
    value.length > 4 ||
    value[0] !== 'M' ||
...
export function UnnormalizeDuration(internalDuration: InternalDuration, largestUnit: Temporal.DateTimeUnit) {
