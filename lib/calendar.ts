...
function CompareSurpasses(sign: -1 | 1, year: number, month: number, day: number, target: ISODate) {
...
function ISODateSurpasses(sign: -1 | 1, baseDate: ISODate, isoDate2: ISODate, years: number, months: number) {
...
abstract class HelperBase {
  ...
  untilCalendar(
    ...
    assert(sign !== 0, 'equal dates should have been checked earlier');
...
function adjustEras(erasParam: InputEra[]): { eras: Era[]; anchorEra: Era } {
  ...
  const moreThanOneReverseOf = eras.filter((e) => e.reverseOf != null).length > 1;
  ...
  eras.forEach((e) => {
    ...
    if (reverseOf) {
      ...
      assert(reversedEra !== undefined, `Invalid era data: unmatched reverseOf era: ${reverseOf}`);
  ...
  eras.sort((e1, e2) => {
    ...
    assert(e1.isoEpoch !== undefined && e2.isoEpoch !== undefined, 'Invalid era data: missing ISO epoch');
...
class NonIsoCalendar implements CalendarImpl {
  ...
  dateUntil(one: ISODate, two: ISODate, largestUnit: Temporal.DateUnit) {
    ...
    if (largestUnit === 'week' || largestUnit === 'day') {
      ...
      let days =
        ES.ISODateToEpochDays(two.year, two.month - 1, two.day) -
        ES.ISODateToEpochDays(one.year, one.month - 1, one.day);
